-- 0017: dash_search accepts an optional p.categories array, matched with lo.category = any(...),
-- alongside the existing single p.category. Lets one page (Coins & Stamps) span more than one
-- classify_lot() bucket the way Watches spans exactly one.
