-- 0013: every case also carries the profit and ROI you would make at the current bid.
--   cost   = current high bid x (1 + buyer's premium)
--   profit = the case value less its resale fee, less that cost
--   ROI    = profit / cost   (no ROI while the bid is $0)
-- dash_search returns profit_* and roi_* for worst, base and best, and sorts by worst_roi, base_roi and best_roi.
-- dash_lot's cases carry proceeds, profit and roi.
...
language sql stable set search_path = public, pg_temp as $$
  select case when p_value is null or p_value <= 0 then null else jsonb_build_object(
    ...
    'profit', (p_value - resale_fee(p_value)) - coalesce(p_bid, 0) * (1 + p.prem),
    'roi', case when coalesce(p_bid, 0) > 0
                then ((p_value - resale_fee(p_value)) - p_bid * (1 + p.prem)) / (p_bid * (1 + p.prem)) end)
  end
  from (select coalesce((select (value #>> '{}')::numeric from settings where key = 'buyer_premium'), 0.25) as prem) p;
$$;
...
drop function if exists case_json(numeric);
