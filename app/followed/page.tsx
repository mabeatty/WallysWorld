...
import { rpc, type RefreshStatus, type Search } from "@/lib/supabase";
...
function Pager({ page, pages, href }: { page: number; pages: number; href: (p: number) => string }) {
  ...
  return (
    <nav className="pages" aria-label="Pages of followed lots">
...
export default async function Followed({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const now = Date.now();
  const counts = { open: openCount.total, closed: closedCount.total, all: allCount.total };
  ...
  return (
    <>
      <header className="top">
        <h1>Followed</h1>
        <nav className="links"><Link href="/">Dashboard</Link><Link href="/all">All items</Link><Link href="/watches">Watches</Link><Link href="/collectibles">Collectibles</Link><Link href="/lots">Find lots</Link><Link href="/catawiki">Catawiki</Link><Link href="/setup">Setup</Link></nav>
      </header>
      ...
      <form method="get" action="/followed" className="search">
        <input type="search" name="q" defaultValue={q} placeholder="Search your followed lots" aria-label="Search followed lots" />
      ...
      <p className="note" style={{ marginTop: 18 }}>
        {r.total === 0
          ? q ? "Nothing matches that search." : "Nothing followed yet. Click the star next to any lot to add it here."
          : `Showing ${from}-${to} of ${r.total.toLocaleString("en-US")} ${word}${noun}${q ? ` matching "${q}"` : ""}.`}
        {r.total > 0 && " Click the star to unfollow. Click a column heading to sort."}
      </p>
